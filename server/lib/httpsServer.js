...
(function () {
  ...
  fs.readFile(path.join(__dirname, '../ssl.crt'), function (err, data) {
    if (err || !data) {
      if (err.code !== 'ENOENT') {
        console.error(err || 'empty HTTPS certificate file');
      }
      fs.readFile(path.join(__dirname, '../default-ssl.crt'), crtCb);
    }
    crtCb(null, data);
  });

  fs.readFile(path.join(__dirname, '../ssl.key'), function (err, data) {
    if (err || !data) {
      if (err.code !== 'ENOENT') {
        console.error(err || 'empty HTTPS key file');
      }
      fs.readFile(path.join(__dirname, '../default-ssl.key'), keyCb);
    }
    keyCb(null, data);
  });
  ...
  function printErr(err, port) {
    console.error('HTTPS port', port, 'server error:', err);
  }
  ...
  function restringifyHeaders(req) {
    ...
    headers += req.method.toUpperCase() + ' ' + req.url + ' ' + 'HTTPS/' + req.httpVersion + '\r\n';
  ...
  function createHttpsListener(callback, port, logPath) {
    var server
      , connections   = []
      , finishedData  = []
      , logSettings   = {}
    ...
    function listenOnServer() {
      ...
      server.on('listening', function () {
        ...
        logSettings.logPath = path.resolve(logPath, 'https', port.toString());
    ...
    if (listeners.hasOwnProperty(port)) {
      callbackWrapper('Already listening for HTTPS on port ' + port);
      return;
    }

    authJoin.when(function (certResults, keyResults) {
      var options = {}
        ;

      if (certResults[0]) {
        callbackWrapper("Couldn't get HTTPS certificate: " + certResults[0].toString());
        return;
      }
      if (keyResults[0]) {
        callbackWrapper("Couldn't get HTTPS key: " + keyResults[0].toString());
        return;
      }

      options.cert = certResults[1];
      options.key = keyResults[1];
  ...
  function changeLogSettings(port, newSettings) {
    ...
    if (!listeners[port]) {
      return {error: 'No HTTPS listener on port ' + port};
    }
  ...
  function closeHttpListener(callback, port) {
    ...
    if (!listeners[port]) {
      callback('No HTTPS listener on specified port ' + port);
      return;
    }
  ...
  module.exports.createListener     = createHttpsListener;
